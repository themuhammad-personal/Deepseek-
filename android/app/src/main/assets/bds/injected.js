...
`.trim(),L=[{name:"xlsx",keywords:["excel","spreadsheet","xlsx","xls","sheet","tabular data","workbook","cells",".xlsx"],skill:J},{name:"pptx",keywords:["powerpoint","presentation","slide","pptx",".pptx","slideshow","deck","power point"],skill:G},{name:"docx",keywords:["word","document","docx","msword","word document","doc",".docx","letter","report"],skill:W}];function z(e){if(!e||typeof e!="string")return[];const t=e.toLowerCase(),n=[];for(const r of L)for(const o of r.keywords)if(t.includes(o)){n.push(r.name);break}return n}function Y(e){const t=z(e);if(!t.length)return"";const n=[];for(const r of t){const o=L.find(a=>a.name===r);o&&n.push(o.skill)}return n.length?["<BetterDeepSeek>","[OFFICE SKILL] The user wants to create an office document. Below is the API reference for the required library:","",n.join(`

`),"</BetterDeepSeek>"].join(`
`):""}const K=new Set(["the","a","an","and","or","but","if","then","else","when","at","by","for","with","about","against","is","it","was","were","are","be","been","between","into","through","during","before","after","above","below","to","from","up","down","in","out","on","off","over","under","again","further","once","here","there","all","any","both","each","few","more","most","other","some","such","no","nor","not","only","own","same","so","than","too","very","can","will","just","should","now","how","what","where","why","who","which","ve","veya","ama","fakat","lakin","ancak","ise","ki","de","da","mi","mu","m\xFC","m\u0131","bir","bu","\u015Fu","o","i\xE7in","gibi","kadar","ile","taraf\u0131ndan","hakk\u0131nda","kar\u015F\u0131","aras\u0131nda","i\xE7ine","boyunca","\xF6nce","sonra","\xFCzerinde","alt\u0131nda","yine","daha","en","t\xFCm","her","baz\u0131","hi\xE7","sadece","kendi","ayn\u0131","\xF6yle","b\xF6yle","\xE7ok","yap\u0131lan","yaparak","olan"]);function V(e,t=800,n=5){if(!e||!e.content)return[];const r=e.content.split(/\r?\n/);if(r.length===0)return[];const o=[];let a=0;for(;a<r.length;){const d=[];let s=0;const h=a+1;for(;a<r.length&&(s<t||d.length<3);)d.push(r[a]),s+=r[a].length+1,a++;const c=a;if(o.push({fileName:e.name,content:d.join(`
`),startLine:h,endLine:c}),a>=r.length)break;a=Math.max(h,a-n)}return o}function N(e){return e?(String(e).toLowerCase().match(/[a-z0-9_\u015f\u00e7g\u00f6\u0131\u00fc]+/gi)||[]).filter(n=>n.length>=2&&!K.has(n)):[]}function Q(e,t,n=5){if(!e||!t||!t.length)return[];const r=[];for(const b of t)r.push(...V(b,800,5));if(r.length===0)return[];const o=N(e);if(o.length===0)return[];const a=r.length,d=r.map(b=>N(b.content)),s=d.map(b=>b.length),c=s.reduce((b,k)=>b+k,0)/a||1,u={};for(const b of o){u[b]=0;for(const k of d)k.includes(b)&&u[b]++}const p=1.2,E=.75,x=[];for(let b=0;b<a;b++){const k=r[b],g=d[b],w=s[b];let i=0;const l={};for(const S of g)l[S]=(l[S]||0)+1;for(const S of o){const m=l[S]||0;if(m===0)continue;const y=u[S]||0,R=Math.log(1+(a-y+.5)/(y+.5))*(m*(p+1))/(m+p*(1-E+E*(w/c)));i+=R}const f=String(k.fileName).toLowerCase();for(const S of o)f.includes(S)&&(i+=12);i>0&&x.push({...k,score:i})}return x.sort((b,k)=>k.score-b.score).slice(0,Math.max(1,n))}function Z(e,t="Project"){if(!e||!e.length)return"";let n=`<BDS:PROJECT_CONTEXT>
...
`,n+=`\`\`\`${o}
`,n+=r.content+`
`,n+="```\n\n"}return n+="</BDS:PROJECT_CONTEXT>",n}function I(e,t){var x,b,k;t.sessionUserMsgCounts||(t.sessionUserMsgCounts={});const n=ee(e),r=te(e);let o=1;n&&n.length>0?(o=n.filter(g=>{const w=String(g.role||g.author||"").toLowerCase();return w==="user"||w==="human"}).length,t.sessionUserMsgCounts[r]=o):typeof e.prompt=="string"&&(e.message_id===1||e.parent_message_id==null?o=1:o=(t.sessionUserMsgCounts[r]||0)+1,t.sessionUserMsgCounts[r]=o);let a=!1,d=null;if(n&&n.length>0){d=O(n)||n[n.length-1];const g=A(d);if(g){const w=j(g),i=ne(n,d);let l=!1;const f=t.config.systemPromptInjectionFrequency||"first";if(f==="always")l=!0;else if(f==="every_x"){const m=t.config.systemPromptInjectionInterval||3;(o-1)%m===0?l=!0:i||(l=!0)}else l=!i,(n.length>1||t.hasInjected&&t.hasInjected(r))&&(l=!1);const S=_(w,r,t,l,n,d);window.dispatchEvent(new CustomEvent("bds:mutation-applied",{detail:JSON.stringify({conversationId:r,injectedText:S||"",userPrompt:w})})),S?(v(d,`${S}

${w}`),a=!0):w!==g&&(v(d,w),a=!0)}}else if(typeof e.prompt=="string"){const g=j(e.prompt),w=e.message_id===1||e.parent_message_id==null,i=t.config.systemPromptInjectionFrequency||"first";let l=!1;if(i==="always")l=!0;else if(i==="every_x"){const S=t.config.systemPromptInjectionInterval||3;(w||(o-1)%S===0)&&(l=!0)}else l=w;const f=_(g,r,t,l,null,null);window.dispatchEvent(new CustomEvent("bds:mutation-applied",{detail:JSON.stringify({conversationId:r,injectedText:f||"",userPrompt:g})})),f?(e.prompt=`${f}

${g}`,a=!0):g!==e.prompt&&(e.prompt=g,a=!0)}const s=(x=t.config)==null?void 0:x.modelInputLimits,h=e.model||((b=e.data)==null?void 0:b.model)||((k=e.chat)==null?void 0:k.model)||"",c=String(h).toLowerCase();let u="instant",p="payload";if(c)c.includes("vision")?u="vision":c.includes("reasoner")||c.includes("deepthink")||c.includes("r1")?u="deepthink":(c.includes("expert")||c.includes("pro"))&&(u="expert");else{const g=Te();g&&(u=g,p="dom")}const E=s?s[u]??163840:163840;if(n&&n.length>0){const g=O(n);if(g){const w=A(g);if(console.warn(`[BDS] Guard check: model="${c}" payload.model=${e.model} source=${p} type=${u} limit=${E} msgLen=${w.length} limits=${JSON.stringify(s)}`),w.length>E){const i=`

...[truncated by Better DeepSeek]...`,l=w.slice(0,E-i.length)+i;v(g,l),a=!0,console.warn(`[BDS] TRUNCATED user message from ${w.length} to ${E} chars`)}}}else if(typeof e.prompt=="string"&&(console.warn(`[BDS] Guard check (prompt): model="${c}" payload.model=${e.model} source=${p} type=${u} limit=${E} msgLen=${e.prompt.length} limits=${JSON.stringify(s)}`),e.prompt.length>E)){const g=`

...[truncated by Better DeepSeek]...`;e.prompt=e.prompt.slice(0,E-g.length)+g,a=!0,console.warn(`[BDS] TRUNCATED prompt from ${e.prompt.length} to ${E} chars`)}return{changed:a,payload:e}}function ee(e){return Array.isArray(e.messages)?e.messages:e.data&&Array.isArray(e.data.messages)?e.data.messages:e.chat&&Array.isArray(e.chat.messages)?e.chat.messages:null}function te(e){return String(e.conversation_id||e.conversationId||e.chat_session_id||e.chat_id||e.id||"default")}function O(e){for(let t=e.length-1;t>=0;t-=1){const n=e[t];if(!n||typeof n!="object")continue;const r=String(n.role||n.author||"").toLowerCase();if(r==="user"||r==="human")return n}return null}function A(e){return e?typeof e.content=="string"?e.content:Array.isArray(e.content)?e.content.map(t=>typeof t=="string"?t:t&&typeof t.text=="string"?t.text:"").join(`
`):typeof e.prompt=="string"?e.prompt:"":""}function v(e,t){if(e){if(typeof e.content=="string"||e.content==null){e.content=t;return}if(Array.isArray(e.content)){e.content=[{type:"text",text:t}];return}if(typeof e.prompt=="string"){e.prompt=t;return}e.content=t}}function ne(e,t=null){if(!Array.isArray(e))return!1;for(const n of e){if(n===t)continue;if(A(n).includes("<BetterDeepSeek>"))return!0}return!1}function _(e,t,n,r=!1,o=null,a=null){var f,S;const d=[],s=re(e,t,n);s&&d.push(s);const h=Ee(n);h&&d.push(h);const c=ke(n);c&&(d.push(c),(f=n.config)!=null&&f.deepCode&&(n.config.deepCode.pendingReport=null),typeof window<"u"&&window.dispatchEvent(new CustomEvent("bds:clear-harness-report")));const u=n.config.systemPromptEntries||[];if(u.length>0){const m=n.sessionUserMsgCounts[t]||1;for(const y of u)y.content.trim()&&ge(y,m,t,n)&&(d.push(`<BetterDeepSeek>
${y.content.trim()}
</BetterDeepSeek>`),n.markEntryInjected&&n.markEntryInjected(t,y.id))}else r&&n.config.systemPrompt.trim()&&!n.config.disableSystemPrompt&&(d.push(`<BetterDeepSeek>
${n.config.systemPrompt.trim()}
</BetterDeepSeek>`),n.markInjected&&n.markInjected(t));const p=P(n.config.skills);let E=null;!r&&o&&(E=ye(o,a));const x=!!(n.config.systemPrompt&&n.config.systemPrompt.trim());if(r&&x||p&&p!==E){const m=ie(n);m&&d.push(m)}const b=ue(e,n,o);b&&d.push(b);const k=Y(e);k&&d.push(k);const g=n.config.activeCharacter;if(g){let m=o?Se(o,a):null;if(!m&&n.getLastChar&&(m=n.getLastChar(t)),!m&&n.currentSessionChar&&(o==null?void 0:o.length)>1&&(m=n.currentSessionChar),r||!m||m!==g.name){const y=pe(n);y&&(d.push(y),n.setLastChar&&n.setLastChar(t,g.name),n.currentSessionChar=g.name)}}n.isNextVoiceMessage&&(d.push("<BetterDeepSeek>User send this message using voice recorder tool.</BetterDeepSeek>"),n.isNextVoiceMessage=!1);const w=n.config&&n.config.activeProject;if(w){let m=null;if(!r&&o&&(m=be(o,a)),r||!m||m!==w.name){const y=he(n);y&&d.push(y)}if(n.config.projectRagEnabled&&Array.isArray(w.files)&&w.files.length>0){const y=Number(n.config.projectRagLimit)||5,T=Q(e,w.files,y);if(T&&T.length>0){const R=Z(T,w.name);R&&d.push(R)}}}if(r){const m=fe(n);m&&d.push(m)}const i=se((S=n.config)==null?void 0:S.mcpToolSchemas);let l=null;if(!r&&o&&(l=we(o,a)),r||i&&i!==l){const m=me(n,i);m&&d.push(m)}return d.join(`
...
`);return`<BetterDeepSeek> <BDS:SKILLS fingerprint="${P(e.config.skills)}">
${t}
</BDS:SKILLS> </BetterDeepSeek>`}function P(e){return!Array.isArray(e)||!e.length?"":e.map(t=>`${t.name}:${(t.content||"").length}`).sort().join("|")}function se(e){return!Array.isArray(e)||!e.length?"":e.map(t=>`${t.serverName}:${t.toolName}:${JSON.stringify(t.inputSchema||{})}`).sort().join("|")}function ae(e){if(!Array.isArray(e))return null;for(let t=e.length-1;t>=0;t--){const n=e[t];if(!n||typeof n!="object")continue;const r=String(n.role||n.author||"").toLowerCase();if(!(r==="user"||r==="human")&&(r==="assistant"||r==="ai"||r==="bot"))return n}return null}function B(e){return!e||typeof e!="string"?[]:e.split(new RegExp("[_-]|\\s+|(?<=[a-z])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])")).map(t=>t.toLowerCase().replace(/[^a-z0-9]/g,"")).filter(t=>t.length>0)}function ce(e,t){if(!e.length||!t.length)return 0;const n=new Set(t);let r=0;for(const o of e)n.has(o)&&r++;return r/e.length}function le(e,t){return t===1?e>=1:e>=.5}function ue(e,t,n){if(t.config.disableMemory||!t.config.memories.length)return"";const r=n?ae(n):null,o=r?A(r):"",a=[e,o].filter(Boolean).join(" "),d=B(a),s=[];for(const c of t.config.memories){if(c.importance==="always"){s.push(c);continue}if(!c.key)continue;const u=B(c.key);if(!u.length){a.toLowerCase().includes(c.key.toLowerCase())&&s.push(c);continue}const p=[...new Set(u)],E=ce(p,d);(le(E,p.length)||a.toLowerCase().includes(c.key.toLowerCase()))&&s.push(c)}return s.length?`<BetterDeepSeek>
...
</BetterDeepSeek>`}function xe(e,t,n,r){const o=window.fetch;window.fetch=async function(d,s){try{const h=Ae(d);if(!t(h))return o.apply(this,arguments);if(Le(d,s,e),h.includes("/api/v0/chat_session/fetch_page")){const c=await o.apply(this,arguments);return c.clone().json().then(p=>{window.dispatchEvent(new CustomEvent("bds:session-data",{detail:JSON.stringify(p)}))}).catch(()=>{}),c}if(h.includes("/api/v0/chat/history_messages")){const c=await o.apply(this,arguments);return c.clone().json().then(p=>{window.dispatchEvent(new CustomEvent("bds:history-msgs",{detail:JSON.stringify(p)}))}).catch(()=>{}),c}n(h);try{const c=await Re(d,s,e);if(!c){const p=await o.apply(this,arguments);return C(p,h,c==null?void 0:c.modelName),p}let u;try{u=await o.call(this,c.input,c.init)}catch(p){return console.warn("[BetterDeepSeek] Mutated fetch failed, retrying original fetch:",p),u=await o.apply(this,arguments),C(u,h,c==null?void 0:c.modelName),u}if(u&&u.status>=400&&u.status<500){console.warn("[BetterDeepSeek] Mutated fetch returned HTTP "+u.status+", retrying original request...");try{const p=await o.apply(this,arguments);return C(p,h,c==null?void 0:c.modelName),p}catch(p){console.error("[BetterDeepSeek] Fallback fetch failed:",p)}}return u&&u.status>=500&&window.dispatchEvent(new CustomEvent("bds:network-error",{detail:JSON.stringify({url:h,status:u.status,type:"fetch"})})),C(u,h,c.modelName),u}catch(c){console.warn("[BetterDeepSeek] Fetch patch inner error, falling back to original fetch:",c);try{return await o.apply(this,arguments)}catch(u){throw window.dispatchEvent(new CustomEvent("bds:network-error",{detail:JSON.stringify({url:h,status:0,type:"fetch",error:String(u)})})),u}}finally{r(h)}}catch(h){return console.warn("[BetterDeepSeek] Request patch failed:",h),o.apply(this,arguments)}}}function C(e,t,n){var r;if(!(!e||!e.clone))try{const o=((r=e.headers)==null?void 0:r.get("content-type"))||"";if(o.includes("text/event-stream")||o.includes("stream")||t&&t.includes("/chat/completion"))return;const a=e.clone();Ce(a,n).catch(()=>{})}catch{}}function Ae(e){return typeof e=="string"?e:e instanceof URL?e.toString():e instanceof Request?e.url:""}async function Re(e,t,n){const r=await De(e,t);if(!r)return null;let o;try{o=JSON.parse(r)}catch{return null}const a=o.model||null,d=I(o,n);if(!d.changed)return null;const s=JSON.stringify(d.payload),h=t&&t.headers?t.headers:e instanceof Request?e.headers:void 0,c=new Headers(h||{});c.set("content-type","application/json");const u={method:t&&t.method||(e instanceof Request?e.method:"POST"),headers:c,body:s,credentials:t&&t.credentials||(e instanceof Request?e.credentials:void 0),cache:t&&t.cache||(e instanceof Request?e.cache:void 0),mode:t&&t.mode||(e instanceof Request?e.mode:void 0),redirect:t&&t.redirect||(e instanceof Request?e.redirect:void 0),referrer:t&&t.referrer||(e instanceof Request?e.referrer:void 0),referrerPolicy:t&&t.referrerPolicy||(e instanceof Request?e.referrerPolicy:void 0),keepalive:t&&t.keepalive||(e instanceof Request?e.keepalive:void 0),integrity:t&&t.integrity||(e instanceof Request?e.integrity:void 0),signal:t&&t.signal||(e instanceof Request?e.signal:void 0)};return{input:typeof e=="string"||e instanceof URL?e:e.url,init:u,modelName:a}}async function Ce(e,t){try{const n=e.headers.get("content-type")||"";if(n.includes("text/event-stream")||n.includes("stream"))await ve(e,t);else{const r=await e.text();try{const o=JSON.parse(r),a=(o==null?void 0:o.usage)||(o==null?void 0:o.token_usage);a&&M(a.prompt_tokens||a.input_tokens||0,a.completion_tokens||a.output_tokens||0,t)}catch{}}}catch{}}async function ve(e,t){var d;const n=(d=e.body)==null?void 0:d.getReader();if(!n)return;const r=new TextDecoder;let o="";try{for(;;){const{done:s,value:h}=await n.read();if(h&&(o+=r.decode(h,{stream:!s})),s)break}}catch{return}const a=o.split(`
`);for(let s=a.length-1;s>=0;s--){const h=a[s].trim();if(!h.startsWith("data: "))continue;const c=h.slice(6).trim();if(c!=="[DONE]")try{const u=JSON.parse(c),p=(u==null?void 0:u.usage)||(u==null?void 0:u.token_usage);if(p){M(p.prompt_tokens||p.input_tokens||0,p.completion_tokens||p.output_tokens||0,t||(u==null?void 0:u.model));break}}catch{}}}function M(e,t,n){typeof e!="number"&&typeof t!="number"||window.dispatchEvent(new CustomEvent("bds:token-usage",{detail:JSON.stringify({inputTokens:Number(e)||0,outputTokens:Number(t)||0,modelName:n||null,timestamp:Date.now()})}))}async function De(e,t){return t&&typeof t.body=="string"?t.body:t&&t.body instanceof URLSearchParams?t.body.toString():e instanceof Request?e.clone().text():""}function Le(e,t,n){try{let r;if(t&&t.headers){const o=t.headers;if(o instanceof Headers)r=o.get("authorization");else if(Array.isArray(o)){for(const[a,d]of o)if(a.toLowerCase()==="authorization"){r=d;break}}else typeof o=="object"&&(r=o.Authorization||o.authorization)}!r&&e instanceof Request&&(r=e.headers.get("authorization")),r&&typeof(n==null?void 0:n.setAuthToken)=="function"&&n.setAuthToken(r)}catch{}}function Ne(e,t,n,r){const o=XMLHttpRequest.prototype.open,a=XMLHttpRequest.prototype.send,d=XMLHttpRequest.prototype.setRequestHeader;XMLHttpRequest.prototype.open=function(h,c){return this.__bdsRequestMeta={method:String(h||"GET").toUpperCase(),url:String(c||"")},o.apply(this,arguments)},XMLHttpRequest.prototype.setRequestHeader=function(h,c){return h&&String(h).toLowerCase()==="authorization"&&typeof(e==null?void 0:e.setAuthToken)=="function"&&e.setAuthToken(String(c||"")),d.apply(this,arguments)},XMLHttpRequest.prototype.send=function(h){try{const c=this.__bdsRequestMeta||{};if(!t(c.url))return a.call(this,h);if(c.url.includes("/api/v0/chat_session/fetch_page"))return this.addEventListener("load",()=>{try{const i=JSON.parse(this.responseText);window.dispatchEvent(new CustomEvent("bds:session-data",{detail:JSON.stringify(i)}))}catch{}}),a.call(this,h);if(c.url.includes("/api/v0/chat/history_messages"))return this.addEventListener("load",()=>{try{const i=JSON.parse(this.responseText);window.dispatchEvent(new CustomEvent("bds:history-msgs",{detail:JSON.stringify(i)}))}catch{}}),a.call(this,h);n(c.url);let u=!1;const p=()=>{u||(u=!0,(this.status>=500||this.status===0)&&window.dispatchEvent(new CustomEvent("bds:network-error",{detail:JSON.stringify({url:c.url,status:this.status,type:"xhr"})})),r(c.url))};this.addEventListener("loadend",p,{once:!0});const E=Ie(h);if(!E)return a.call(this,h);const x=JSON.parse(E),b=x.model||null,k=I(x,e);if(!k.changed)return a.call(this,h);const g=JSON.stringify(k.payload),w=this;return this.addEventListener("load",()=>{try{const i=w.responseText;i&&Oe(i,w,b)}catch{}},{once:!0}),a.call(this,g)}catch(c){const u=this.__bdsRequestMeta||{};console.warn("[BetterDeepSeek] XHR patch failed:",c);try{return a.call(this,h)}catch(p){throw t(u.url)&&r(u.url),p}}}}function Ie(e){return typeof e=="string"?e:e instanceof URLSearchParams?e.toString():""}function Oe(e,t,n){var r;try{if((((r=t.getResponseHeader)==null?void 0:r.call(t,"content-type"))||"").includes("text/event-stream")||e.startsWith("data: ")){const a=e.split(`
`);for(let d=a.length-1;d>=0;d--){const s=a[d].trim();if(!s.startsWith("data: "))continue;const h=s.slice(6).trim();if(h!=="[DONE]")try{const c=JSON.parse(h),u=c==null?void 0:c.usage;if(u){window.dispatchEvent(new CustomEvent("bds:token-usage",{detail:JSON.stringify({inputTokens:u.prompt_tokens||u.input_tokens||0,outputTokens:u.completion_tokens||u.output_tokens||0,modelName:n||(c==null?void 0:c.model)||null,timestamp:Date.now()})}));break}}catch{}}}}catch{}}(function(){"use strict";const e={configUpdate:"bds:config-update",deepResearchConfigUpdate:"bds:deep-research-config-update",requestConfig:"bds:request-config",markVoiceMessage:"bds:mark-voice-message",sessionData:"bds:session-data"},t="/api/v0/chat_session/fetch_page",n="/api/v0/chat/history_messages",r="/api/v0/chat/completion";function o(){try{return JSON.parse(localStorage.getItem("bds_injected_chats")||"[]")}catch{return[]}}function a(i){const l=o();l.includes(i)||(l.push(i),l.length>50&&l.shift(),localStorage.setItem("bds_injected_chats",JSON.stringify(l)))}function d(){try{return JSON.parse(localStorage.getItem("bds_injected_chars")||"{}")}catch{return{}}}function s(i,l){const f=d();f[i]=l;const S=Object.keys(f);S.length>50&&delete f[S[0]],localStorage.setItem("bds_injected_chars",JSON.stringify(f))}function h(i){try{return JSON.parse(localStorage.getItem("bds_injected_entries")||"{}")[i]||[]}catch{return[]}}function c(i,l){try{const f=JSON.parse(localStorage.getItem("bds_injected_entries")||"{}");f[i]||(f[i]=[]),f[i].includes(l)||f[i].push(l);const S=Object.keys(f);S.length>50&&delete f[S[0]],localStorage.setItem("bds_injected_entries",JSON.stringify(f))}catch{}}function u(){var i,l;try{for(let S=0;S<localStorage.length;S++){const m=localStorage.key(S);if(m&&/token|auth|session/i.test(m)){const y=localStorage.getItem(m);if(!y)continue;if(y.trim().startsWith("{"))try{const T=JSON.parse(y),R=T.token||T.accessToken||T.access_token||T.user_token||((i=T.user)==null?void 0:i.token);if(R&&typeof R=="string")return R}catch{}else if(typeof y=="string"&&y.length>20){let T=y;return T.startsWith("Bearer ")&&(T=T.substring(7)),T.startsWith('"')&&T.endsWith('"')&&(T=T.slice(1,-1)),T}}}const f=(l=document.cookie.split("; ").find(S=>S.startsWith("user_token=")||S.startsWith("token=")))==null?void 0:l.split("=")[1];if(f)return decodeURIComponent(f)}catch(f){console.warn("[BDS] Failed to search auth token in storage:",f)}return null}const p={config:{systemPrompt:"",systemPromptEntries:[],skills:[],memories:[],activeCharacter:null,mcpToolSchemas:[]},hasInjected:i=>o().includes(i),markInjected:i=>a(i),getInjectedEntries:i=>h(i),markEntryInjected:(i,l)=>c(i,l),getLastChar:i=>d()[i]||null,setLastChar:(i,l)=>s(i,l),currentSessionChar:null,activeCompletionRequests:0,isNextVoiceMessage:!1,authToken:u(),setAuthToken:function(i){i&&i!==this.authToken&&(this.authToken=i)}};if(window.__bdsNetworkPatched)return;window.__bdsNetworkPatched=!0,(function(){if(window.__BDS_CONFIG__)return;let i=0;const l=new Map;window.addEventListener("bds:debug-api-response",S=>{let m=S.detail;if(typeof m=="string")try{m=JSON.parse(m)}catch{return}const y=l.get(m.id);y&&(y(m.result),l.delete(m.id))});function f(S){return function(){const m=Array.from(arguments);return new Promise(y=>{const T=++i;l.set(T,y),window.dispatchEvent(new CustomEvent("bds:debug-api-request",{detail:JSON.stringify({id:T,method:S,args:m})}))})}}window.__BDS_CONFIG__={raw:f("getRaw"),getFlag:f("getFlag"),getConfig:f("getConfig"),applyRemote:f("applyRemote"),replaceRemote:f("replaceRemote"),resetToBuiltin:f("resetToBuiltin"),detectModel:f("detectModel"),toggleDebugPanel:f("toggleDebugPanel")}})(),window.addEventListener(e.configUpdate,i=>{let l=i&&i.detail?i.detail:{};if(typeof l=="string")try{l=JSON.parse(l)}catch(f){console.error("[BDS] Failed to parse configUpdate detail:",f)}p.config=H(l||{})}),window.addEventListener(e.deepResearchConfigUpdate,i=>{let l=i&&i.detail?i.detail:{};if(typeof l=="string")try{l=JSON.parse(l)}catch(f){console.error("[BDS] Failed to parse deepResearchConfigUpdate detail:",f)}p.config.deepResearch=D(l||{})}),window.addEventListener(e.markVoiceMessage,()=>{p.isNextVoiceMessage=!0}),window.addEventListener("bds:request-history-msgs",async i=>{let l=i&&i.detail?i.detail:{};if(typeof l=="string")try{l=JSON.parse(l)}catch{return}const f=l==null?void 0:l.sessionId;if(!f)return;const S=`${n}?chat_session_id=${encodeURIComponent(f)}`,m={"Content-Type":"application/json"};p.authToken&&(m.Authorization=`Bearer ${p.authToken}`);try{const y=await E(S,{method:"GET",headers:m,credentials:"include"});if(!y.ok){console.warn("[BDS] history_mgs fetch failed:",y.status);return}const T=await y.json();T.__bdsExplicit=!0,window.dispatchEvent(new CustomEvent("bds:history-msgs",{detail:JSON.stringify(T)}))}catch(y){console.warn("[BDS] history_msgs fetch error:",y)}}),x();const E=window.fetch.bind(window);xe(p,b,g,w),Ne(p,b,g,w);function x(){window.dispatchEvent(new CustomEvent(e.requestConfig))}function b(i){const l=String(i||"");return l.includes("/api/v0/chat/completion")||l.includes("/api/v0/chat/edit_message")||l.includes(t)||l.includes(n)}function k(i,l){const f={status:i,url:String(l||""),activeCompletionRequests:p.activeCompletionRequests,timestamp:Date.now()};window.dispatchEvent(new CustomEvent(e.networkState,{detail:JSON.stringify(f)}))}function g(i){p.activeCompletionRequests+=1,k("start",i)}function w(i){p.activeCompletionRequests=Math.max(0,p.activeCompletionRequests-1),k("end",i)}})()})();
