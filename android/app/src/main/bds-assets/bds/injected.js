...
slide1.addText("Prepared by Super DeepSeek", { x: 1, y: 3.5, w: 8, h: 0.8, fontSize: 16, align: "center" });
...
${g}`,a=!0):g!==e.prompt&&(e.prompt=g,a=!0)}const s=(x=t.config)==null?void 0:x.modelInputLimits,f=e.model||((y=e.data)==null?void 0:y.model)||((k=e.chat)==null?void 0:k.model)||"",c=String(f).toLowerCase();let h="instant",m="payload";if(c)c.includes("vision")?h="vision":c.includes("reasoner")||c.includes("deepthink")||c.includes("r1")?h="deepthink":(c.includes("expert")||c.includes("pro"))&&(h="expert");else{const g=Te();g&&(h=g,m="dom")}const E=s?s[h]??163840:163840;if(n&&n.length>0){const g=N(n);if(g){const w=A(g);if(console.warn(`[SDS] Guard check: model="${c}" payload.model=${e.model} source=${m} type=${h} limit=${E} msgLen=${w.length} limits=${JSON.stringify(s)}`),w.length>E){const i=`

...[truncated by Super DeepSeek]...`,l=w.slice(0,E-i.length)+i;C(g,l),a=!0,console.warn(`[SDS] TRUNCATED user message from ${w.length} to ${E} chars`)}}}else if(typeof e.prompt=="string"&&(console.warn(`[SDS] Guard check (prompt): model="${c}" payload.model=${e.model} source=${m} type=${h} limit=${E} msgLen=${e.prompt.length} limits=${JSON.stringify(s)}`),e.prompt.length>E)){const g=`

...[truncated by Super DeepSeek]...`;e.prompt=e.prompt.slice(0,E-g.length)+g,a=!0,console.warn(`[SDS] TRUNCATED prompt from ${e.prompt.length} to ${E} chars`)}return{changed:a,payload:e}}function ee(e){return Array.isArray(e.messages)?e.messages:e.data&&Array.isArray(e.data.messages)?e.data.messages:e.chat&&Array.isArray(e.chat.messages)?e.chat.messages:null}function te(e){return String(e.conversation_id||e.conversationId||e.chat_session_id||e.chat_id||e.id||"default")}function N(e){for(let t=e.length-1;t>=0;t-=1){const n=e[t];if(!n||typeof n!="object")continue;const r=String(n.role||n.author||"").toLowerCase();if(r==="user"||r==="human")return n}return null}function A(e){return e?typeof e.content=="string"?e.content:Array.isArray(e.content)?e.content.map(t=>typeof t=="string"?t:t&&typeof t.text=="string"?t.text:"").join(`
`):typeof e.prompt=="string"?e.prompt:"":""}function C(e,t){if(e){if(typeof e.content=="string"||e.content==null){e.content=t;return}if(Array.isArray(e.content)){e.content=[{type:"text",text:t}];return}if(typeof e.prompt=="string"){e.prompt=t;return}e.content=t}}function ne(e,t=null){if(!Array.isArray(e))return!1;for(const n of e){if(n===t)continue;if(/<(?:Better|Super)DeepSeek>/.test(A(n)))return!0}return!1}function O(e,t,n,r=!1,o=null,a=null){var l,p;const u=[],s=re(e,t,n);s&&u.push(s);const f=Ee(n);f&&u.push(f);const c=ke(n);c&&(u.push(c),(l=n.config)!=null&&l.deepCode&&(n.config.deepCode.pendingReport=null),typeof window<"u"&&window.dispatchEvent(new CustomEvent("bds:clear-harness-report")));const h=n.config.systemPromptEntries||[];if(h.length>0){const d=n.sessionUserMsgCounts[t]||1;for(const S of h)S.content.trim()&&ge(S,d,t,n)&&(u.push(`<SuperDeepSeek>
${S.content.trim()}
</SuperDeepSeek>`),n.markEntryInjected&&n.markEntryInjected(t,S.id))}else r&&n.config.systemPrompt.trim()&&!n.config.disableSystemPrompt&&(u.push(`<SuperDeepSeek>
${n.config.systemPrompt.trim()}
</SuperDeepSeek>`),n.markInjected&&n.markInjected(t));const m=_(n.config.skills);let E=null;if(!r&&o&&(E=ye(o,a)),r||m&&m!==E){const d=ie(n);d&&u.push(d)}const x=ue(e,n,o);x&&u.push(x);const y=Y(e);y&&u.push(y);const k=n.config.activeCharacter;if(k){let d=o?Se(o,a):null;if(!d&&n.getLastChar&&(d=n.getLastChar(t)),!d&&n.currentSessionChar&&(o==null?void 0:o.length)>1&&(d=n.currentSessionChar),r||!d||d!==k.name){const S=fe(n);S&&(u.push(S),n.setLastChar&&n.setLastChar(t,k.name),n.currentSessionChar=k.name)}}n.isNextVoiceMessage&&(u.push("<SuperDeepSeek>User send this message using voice recorder tool.</SuperDeepSeek>"),n.isNextVoiceMessage=!1);const g=n.config&&n.config.activeProject;if(g){let d=null;if(!r&&o&&(d=be(o,a)),r||!d||d!==g.name){const S=he(n);S&&u.push(S)}if(n.config.projectRagEnabled&&Array.isArray(g.files)&&g.files.length>0){const S=Number(n.config.projectRagLimit)||5,b=Q(e,g.files,S);if(b&&b.length>0){const T=Z(b,g.name);T&&u.push(T)}}}if(r){const d=pe(n);d&&u.push(d)}const w=se((p=n.config)==null?void 0:p.mcpToolSchemas);let i=null;if(!r&&o&&(i=we(o,a)),r||w&&w!==i){const d=me(n,w);d&&u.push(d)}return u.join(`

`)}function re(e,t,n){var o;const r=(o=n.config)==null?void 0:o.deepResearch;return!(r!=null&&r.enabled)||!r.runId?"":(r.enabled=!1,oe(r.runId,t,e),["<SuperDeepSeek>",'[SDS:DEEP_RESEARCH] The DeepResearch toggle is enabled. Treat this exactly as the user asking: "Perform Deep Research on the following request."',`Run ID: ${r.runId}`,"","CRITICAL: In this first turn, you must ONLY produce a research plan. Do NOT browse or search. Do NOT produce an ordinary answer. Do NOT produce a direct report.",`Output ONLY a plan using: <SDS:DEEP_RESEARCH_PLAN runId="${r.runId}">JSON</SDS:DEEP_RESEARCH_PLAN>`,"After this turn, SDS will execute steps one-by-one. After each step result is provided, analyze it before continuing. Do NOT skip ahead to the final report until SDS tells you all steps are complete.","","The JSON plan must include:",'- "title": A short descriptive title for the research','- "steps": An array of research steps, each with:','  - "id": step number','  - "action": "search" or "fetch"','  - "query": a specific search query or URL to fetch','  - "purpose": why this step is needed','  - "sourceType": for search steps, one of "general", "docs", "news", "reviews", "academic", or "commerce"',"","Search steps must use narrow queries with named entities, constraints, dates or locations, product or version names, and clear source intent.","",`User research question: ${e}`,"</SuperDeepSeek>"].join(`
...
`);return`<SuperDeepSeek> <SDS:SKILLS fingerprint="${_(e.config.skills)}">
${t}
</SDS:SKILLS> </SuperDeepSeek>`}function _(e){return!Array.isArray(e)||!e.length?"":e.map(t=>`${t.name}:${(t.content||"").length}`).sort().join("|")}function se(e){return!Array.isArray(e)||!e.length?"":e.map(t=>`${t.serverName}:${t.toolName}:${JSON.stringify(t.inputSchema||{})}`).sort().join("|")}function ae(e){if(!Array.isArray(e))return null;for(let t=e.length-1;t>=0;t--){const n=e[t];if(!n||typeof n!="object")continue;const r=String(n.role||n.author||"").toLowerCase();if(!(r==="user"||r==="human")&&(r==="assistant"||r==="ai"||r==="bot"))return n}return null}function P(e){return!e||typeof e!="string"?[]:e.split(new RegExp("[_-]|\\s+|(?<=[a-z])(?=[A-Z])|(?<=[A-Z])(?=[A-Z][a-z])")).map(t=>t.toLowerCase().replace(/[^a-z0-9]/g,"")).filter(t=>t.length>0)}function ce(e,t){if(!e.length||!t.length)return 0;const n=new Set(t);let r=0;for(const o of e)n.has(o)&&r++;return r/e.length}function le(e,t){return t===1?e>=1:e>=.5}function ue(e,t,n){if(t.config.disableMemory||!t.config.memories.length)return"";const r=n?ae(n):null,o=r?A(r):"",a=[e,o].filter(Boolean).join(" "),u=P(a),s=[];for(const c of t.config.memories){if(c.importance==="always"){s.push(c);continue}if(!c.key)continue;const h=P(c.key);if(!h.length){a.toLowerCase().includes(c.key.toLowerCase())&&s.push(c);continue}const m=[...new Set(h)],E=ce(m,u);(le(E,m.length)||a.toLowerCase().includes(c.key.toLowerCase()))&&s.push(c)}return s.length?`<SuperDeepSeek>
${s.map(c=>`<SDS:memory_calls importance="${c.importance}">${c.key}: ${de(c.value)}</SDS:memory_calls>`).join(`
`)}
</SuperDeepSeek>`:""}function de(e){return String(e).replace(/<\//g,"<\\/").trim()}function he(e){const t=e.config&&e.config.activeProject;if(!t)return"";let n="";return t.instructions&&t.instructions.trim()&&(n+=t.instructions.trim()+`
`),`<SuperDeepSeek>
<SDS:PROJECT name="${t.name}">
${n}</SDS:PROJECT>
</SuperDeepSeek>`}function fe(e){const t=e.config.activeCharacter;if(!t||!t.content)return"";let n=`Character Name: ${t.name}
`;return t.usage&&(n+=`Usage Domain: ${t.usage}
`),n+=`---
${t.content.trim()}`,`<SuperDeepSeek> <SDS:RP>
${n}
</SDS:RP> </SuperDeepSeek>`}function pe(e){const t=[];if(e.config.injectSystemDateTime!==!1){const r=new Date;t.push(`User's System Date & Time: ${r.toLocaleString()}`)}const n=e.config.preferredLang;return n&&n.trim()&&t.push(`Always respond in ${n.trim()}.`),t.length===0?"":`<SuperDeepSeek>
${t.join(`
`)}
</SuperDeepSeek>`}function me(e,t){var w;const n=(w=e.config)==null?void 0:w.mcpToolSchemas;if(!Array.isArray(n)||!n.length)return"";const r=Number(e.config.mcpInlineMaxChars)||8e3,o=n.length,a=[`<SuperDeepSeek> <SDS:MCP fingerprint="${t}">`,"You have access to the following MCP (Model Context Protocol) tools via remote servers.",`To invoke them, use: <SDS:AUTO:MCP url="SERVER_NAME_OR_URL" tool="TOOL_NAME" args='{"key":"value"}'>`,'If the arguments contain quotation marks, apostrophes, angle brackets or newlines, send them as base64Args="<url-safe base64 of the JSON>" instead of args.',"The app will call the tool and inject the result.","Important: Only ONE tool per response. Wait for the result before invoking another. Never invoke multiple tools at the same time.","",...(n.some(i=>/^sandbox/i.test(String(i.serverUrl||""))||String(i.serverName||"")==="sandbox")?["","## Your Linux sandbox (server \"sandbox\")","You have your own Linux computer inside this app: Alpine Linux, you are root, internet works, and files persist in /root/workspace between messages. Use it to actually do the work (write code, install tools, run and test it) instead of only describing steps.","Call it with the JSON arguments in the tag body, for example:","<SDS:AUTO:MCP url=\"sandbox\" tool=\"run\">{\"command\":\"python3 --version\"}</SDS:AUTO:MCP>","<SDS:AUTO:MCP url=\"sandbox\" tool=\"write_file\">{\"path\":\"app/main.py\",\"content\":\"print('hello')\\n\"}</SDS:AUTO:MCP>","Rules: the body must be valid JSON (escape quotes, backslashes and newlines as \\\" \\\\ \\n). Never put the tag inside a code block or quote. Exactly one tool call per reply, at the end of the reply; its result arrives as the next message.","Work like an autonomous senior engineer: make a short plan, then act step by step without asking for permission. After every result decide the next step yourself. Verify by running the code or tests. When something fails, read the error, fix the cause and retry. Keep going until the task is completely finished, then reply with a short summary (what you did, where the files are, how to run it) and no tool call.","Prefer edit_file for small changes to existing files. Start servers and watchers with run background=true, check them with job, and show web apps with preview. Install missing tools with install_packages (apk) or pip/npm. Long outputs are cut in the middle, so filter with grep, head or tail.",""]:[]),"Available tools:"].join(`
`),u="</SDS:MCP> </SuperDeepSeek>",s=n.map(i=>{let l=`- Server: ${i.serverName} (${i.serverUrl||i.serverName}) | Tool: ${i.toolName}`;if(i.description&&(l+=` | Description: ${i.description}`),i.inputSchema&&typeof i.inputSchema=="object"){const p=i.inputSchema.properties;if(p){const d=Object.entries(p).map(([S,b])=>{const T=(i.inputSchema.required||[]).includes(S)?" (required)":"";return`${S}: ${(b==null?void 0:b.type)||"any"}${T}`});d.length&&(l+=` | Params: ${d.join(", ")}`)}}return l}),f=[a,...s,u].join(`
...
`)}return g}function ge(e,t,n,r){const a=(r.getInjectedEntries?r.getInjectedEntries(n):[]).includes(e.id);switch(e.schedule.type){case"first":return!a;case"always":return!0;case"interval":{const u=e.schedule.everyNTurns||3;return a?(t-1)%u===0:!0}default:return!1}}function Se(e,t=null){if(!Array.isArray(e))return null;for(let n=e.length-1;n>=0;n--){const r=e[n];if(r===t)continue;const o=A(r);if(!/<[BS]DS:RP>/.test(o))continue;const a=o.match(/Character Name:\s*(.*?)\n/);if(a&&a[1])return a[1].trim()}return null}function ye(e,t=null){if(!Array.isArray(e))return null;for(let n=e.length-1;n>=0;n--){const r=e[n];if(r===t)continue;const a=A(r).match(/<[BS]DS:SKILLS fingerprint="(.*?)">/);if(a&&a[1])return a[1]}return null}function we(e,t=null){if(!Array.isArray(e))return null;for(let n=e.length-1;n>=0;n--){const r=e[n];if(r===t)continue;const a=A(r).match(/<[BS]DS:MCP fingerprint="(.*?)">/);if(a&&a[1])return a[1]}return null}function be(e,t=null){if(!Array.isArray(e))return null;for(let n=e.length-1;n>=0;n--){const r=e[n];if(r===t)continue;const a=A(r).match(/<[BS]DS:PROJECT name="(.*?)">/);if(a&&a[1])return a[1]}return null}function B(e){let t=String(e||"");return t=t.replace(/<(?:Better|Super)DeepSeek>([\s\S]*?)<\/(?:Better|Super)DeepSeek>/gi,(n,r)=>/\[[BS]DS:AUTO\]/.test(r)||/\[[BS]DS:DEEP_RESEARCH\]/.test(r)||/<[BS]DS:memory_calls[\s>]/i.test(r)?n:""),t=t.replace(/<[BS]DS:SKILLS>[\s\S]*?<\/[BS]DS:SKILLS>/gi,""),t=t.replace(/<[BS]DS:memory_calls[^>]*>[\s\S]*?<\/[BS]DS:memory_calls>/gi,""),t=t.replace(/<[BS]DS:RP>[\s\S]*?<\/[BS]DS:RP>/gi,""),t=t.replace(/<[BS]DS:PROJECT[^>]*>[\s\S]*?<\/[BS]DS:PROJECT>/gi,""),t=t.replace(/<[BS]DS:PROJECT_CONTEXT>[\s\S]*?<\/[BS]DS:PROJECT_CONTEXT>/gi,""),t.trim()}function Te(){try{const e=document.querySelector("._46a12ab");if(!e)return null;const t=(e.textContent||"").toLowerCase().trim();return t.includes("vision")?"vision":t.includes("expert")||t.includes("reasoner")?"expert":t.includes("deepthink")||t.includes("deep think")||t.includes("r1")?"deepthink":t.includes("instant")||t.includes("chat")||t.includes("flash")?"instant":null}catch{return null}}function Ee(e){const t=e&&e.config&&e.config.deepCode;if(!t||!t.enabled)return"";const n=t.manualPath||t.activeDirectory||"active directory",r=t.fileTree?`
...
3. SEARCH CODEBASE
   <SDS:AUTO:SEARCH_IN_DIRECTORY queries="query terms"/>
...
4. DISPATCH HARNESS TASK
   <SDS:HARNESS_TASK cwd="${n}">
   ...task spec...
   </SDS:HARNESS_TASK>
   Terminal action. Once emitted, the task is sent for execution. Never emit
   more than one SDS:HARNESS_TASK block per dispatch, and never emit it
...
PHASE 5 \u2014 Dispatch
Once confirmed, emit exactly one SDS:HARNESS_TASK block built to the spec
below.

\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
DISPATCH GATE \u2014 do not emit SDS:HARNESS_TASK unless ALL of these hold
\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
...
\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
TASK SPEC FORMAT (contents of SDS:HARNESS_TASK)
\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550\u2550
...
- Never emit SDS:HARNESS_TASK mid-explanation. It is always the final action
  of a turn.
</SuperDeepSeek>`}function ke(e){const t=e&&e.config&&e.config.deepCode,n=t&&t.pendingReport;if(!n||!n.report||!n.report.trim())return"";const r=n.cwd?` cwd="${n.cwd}"`:"",o=n.sessionId?` sessionId="${n.sessionId}"`:"";return`<SuperDeepSeek>
