# Better DeepSeek (Android Standalone Edition)

[![Android Build](https://img.shields.io/badge/Platform-Android_APK-3DDC84?style=for-the-badge&logo=android&logoColor=white)](#build-apk)
[![Release CI](https://img.shields.io/badge/CI%2FCD-GitHub_Actions-2088FF?style=for-the-badge&logo=github-actions&logoColor=white)](#github-actions-cicd)
[![License](https://img.shields.io/badge/License-MIT-green?style=for-the-badge)](LICENSE)

**Better DeepSeek Android** is an exclusive, standalone Android application designed to bring powerful agentic AI capabilities, custom tools, persistent memory, and Model Context Protocol (MCP) servers directly to your Android device with a modern, fluid mobile user experience inspired by Claude and ChatGPT.

> "Better DeepSeek" is an unofficial, independent, and community-driven open-source application. It is NOT affiliated with, endorsed by, sponsored by, or officially connected to DeepSeek or DeepSeek AI in any way.

---

## 🌟 What's New in this Standalone Android Edition

1. **Brand-New Claude & ChatGPT-Inspired Mobile UI/UX**
   - **Modern Bottom Sheet Container**: Replaces the old desktop extension drawer with an ergonomic mobile bottom sheet featuring drag-handle, blur backdrop, and safe-area responsive padding.
   - **Segmented Category Navigation**: Swiftly switch across dedicated, clutter-free tabs:
     - 🔌 **MCP & Tools**: Interactive server cards, connection health badges, live tool counts, and expandable schemas.
     - 💬 **Chat & AI**: Thinking process toggles, markdown rendering modes, auto-rename controls, and streaming configurations.
     - ⚡ **Prompts & Skills**: System prompts, custom personas, web skill managers, and persistent memories.
     - 📁 **Projects**: Workspaces, workspace file trees, attachments, and directory contexts.
     - ⌨️ **Commands**: Command palette shortcuts, hotkeys, and snippet management.
     - ⚙️ **Settings**: Localization, themes, font sizing, and cloud/local backup imports & exports.
   - **Instant Quick-Filter**: Real-time fuzzy search bar instantly pinpoints any prompt, skill, or configuration toggle.
   - **Zero Feature Loss**: Every tool, setting, preset prompt, and visualization engine from the original project remains intact and completely functional.

2. **Native Custom MCP (Model Context Protocol) Support on Android**
   - **Native OkHttp Bridge**: Fixed the missing MCP handlers (`bds-mcp-list-tools` and `bds-mcp-call`) directly in `WebViewBridge.kt`.
   - **JSON-RPC 2.0 & SSE Streaming**: Full support for both standard HTTP POST JSON-RPC 2.0 and Server-Sent Events (SSE) transports.
   - **Local Cleartext & LAN Support**: Preconfigured `network_security_config.xml` enables seamless connections to local MCP servers running on `10.0.2.2`, `127.0.0.1`, or local WiFi LAN IPs (`192.168.x.x`).
   - **Flexible Authentication**: Out-of-the-box support for Bearer tokens, `X-API-Key` headers, and URL query authentication.

3. **Automated CI/CD via GitHub Actions**
   - Dedicated workflow (`.github/workflows/build-and-release-apk.yml`) that automatically builds, tests, signs, and releases production APKs on every push to `main` and version tag (`v*`).
   - Automatic fallback debug signing ensuring local and CI builds succeed without requiring manual secret configuration.
   - Generates SHA256 verification checksums for every build artifact.
